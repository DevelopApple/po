...
- (void)share {
	UIAlertController *alertController = [UIAlertController
	    alertControllerWithTitle:@"PowerXS"
	...
	[alertController addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
		...
		if([themeField.text isEqual: @""]) {
			...
			NSMutableDictionary *settings = [[NSMutableDictionary alloc] initWithContentsOfFile:@"/var/mobile/Library/Preferences/com.yakir.powerxsprefs.plist"];
			...
			NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"https://themes.yakir.com/publish/powerxs/%@", base64Encoded]];
    			...
    			completionHandler: ^(NSData *data, NSURLResponse *response, NSError *error) {
        	        if (data == nil) {
						...
						if([[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] isEqual: @"Uploaded!"]) {
							...
							UIAlertController *alertController = [UIAlertController
	   							alertControllerWithTitle:@"PowerXS"
	    						message: [NSString stringWithFormat:@"Uploaded!\nLink: https://themes.yakir.com/theme/powerxs/%@", themeField.text]
	    						preferredStyle:UIAlertControllerStyleAlert
							];
							...
							[alertController addAction:[UIAlertAction actionWithTitle:@"Copy Link to Clipboard" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
								...
								pasteboard.string = [NSString stringWithFormat:@"https://themes.yakir.com/theme/powerxs/%@", themeField.text];
							}]];
...
- (void)import {
	...
	[alertController addAction:[UIAlertAction actionWithTitle:@"Submit" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
		...
		if(length < 48 && ![[urlfield.text substringToIndex:48] isEqualToString:@"https://themes.yakir.com/theme/powerxs/"]) {
    		UIAlertController *alertController = [UIAlertController
	   			alertControllerWithTitle:@"PowerXS"
