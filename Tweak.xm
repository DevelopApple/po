...
NSBundle* bundle = [NSBundle bundleWithPath:@"/Library/PreferenceBundles/PowerXSPrefs.bundle"];
...
rebootButton.tintColor = LCPParseColorString(@"#01d890", @"#01d890");
} else if (blurStyle == 2) {
...
powerButton.tintColor = LCPParseColorString(@"#c8392d", @"#c8392d");
} else if (blurStyle == 2) {
...
respringButton.tintColor = LCPParseColorString(@"#ffffff", @"#fffff");
} else if (blurStyle == 2) {
respringButton.tintColor = [UIColor whiteColor];
}
} else if(style != 0) {
...
safeModeButton.tintColor = LCPParseColorString(@"#f1c100", @"#f1c100");
} else if (blurStyle == 2) {
...
HBPreferences *colorPrefs = [[HBPreferences alloc] initWithIdentifier:@"com.yakir.powerxsprefs-colors"];
[colorPrefs registerDefaults:@{
@"kBackgroundColor": @"#FFFFFF",
@"kTintColor": @"#007AFF"
}];

HBPreferences *preferences = [[HBPreferences alloc] initWithIdentifier:@"com.yakir.powerxsprefs"];
...
defaultColor = [preferences integerForKey:@"kDefaultColor"];
...
*/
dpkgInvalid = ![[NSFileManager defaultManager] fileExistsAtPath:@"/var/lib/dpkg/info/com.yakir.powerxs.list"];
...
loadPrefs();
CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), NULL, (CFNotificationCallback)loadPrefs, CFSTR("com.yakir.powerxsprefs/saved"), NULL, CFNotificationSuspensionBehaviorCoalesce);
}
